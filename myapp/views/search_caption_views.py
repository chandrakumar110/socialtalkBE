...
class SearchCaptionView(APIView):
    ...
    def post(self, request):
        if request.method == 'POST' :
            ...
            caption = request.data.get("caption_text")
            if not caption:
                return Response({'error': 'caption_text param is required'}, status=status.HTTP_400_BAD_REQUEST)

            posts = []
            try:
                post_queryset = Posts.objects.filter(caption__icontains = caption)
